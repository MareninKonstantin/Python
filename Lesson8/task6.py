...
class warehouse(ABC):
    ...
    def new_eq(self, eq, count):      
        ...
        if self.dic_wrh.get(self.firm) is None:
            self.dic_wrh.update({self.firm : {self.model : self.count}})
        else:
            temp_dict = self.dic_wrh.get(self.firm)
            if temp_dict.get(self.model) is None:
                temp_dict.update({self.model : self.count})
            else:
                temp_dict.update({self.model : temp_dict.get(self.model) + self.count})
    ...
    def count(self, count):
        if type(count) != int:
            print('Введено некорректное число! Исправим на 1!')
            self.__count = 1
        else:
            self.__count = count
...
class1.new_eq(eq_copier, 'qwe')
class1.new_eq(eq_printer, 10)
print(class1)

class1.to_company('Sberbank', eq_printer, 'qwe')
print(class1)
